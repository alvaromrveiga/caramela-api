...
describe("Show All Consultations controller", () => {
  beforeAll(async () => {
    if (!connection) {
  ...
  it("Should not show other user pet consultations", async () => {
    await request(app)
      .get(`/pets/${otherUserPetId}/consultations`)
      .set({ Authorization: `Bearer ${token}` })
      .send()
      .expect(404);
  });

  it("Should not show user pet consultations if unauthenticated", async () => {
    await request(app).get(`/pets/${petId}/consultations`).send().expect(401);
  });
});
