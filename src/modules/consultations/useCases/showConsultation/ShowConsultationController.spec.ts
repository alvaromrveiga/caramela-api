...
describe("Show Consultation controller", () => {
  beforeAll(async () => {
    if (!connection) {
  ...
  beforeEach(async () => {
    ...
    otherUserConsultationId = response.body.id;
  ...
  it("Should not show consultation if unauthenticated", async () => {
    await request(app)
      .get(`/consultations/${consultationId}`)
      .send()
      .expect(401);
  ...
  it("Should not show invalid consultation", async () => {
    await request(app)
      .get(`/consultations/301c99d5-0840-42a4-9d0a-87ebda4d8c18`)
      .set({ Authorization: `Bearer ${token}` })
      .send()
      .expect(404);
  });
  ...
  it("Should not show other user consultation", async () => {
    await request(app)
      .get(`/consultations/${otherUserConsultationId}`)
      .set({ Authorization: `Bearer ${token}` })
      .send()
      .expect(404);
  });
